...
if [[ <LICENSE TYPE> == "bigiq" ]]; then
    ...
    CLPV2_KEY=`curl -sku 'azureuser:B!giq2017' https://${BIGIQ_ADDRESS}/mgmt/cm/device/licensing/pool/utility/licenses/ | jq -r '.items[] | select(.name=="production")' | jq -r .regKey`

...
curl -k https://s3.amazonaws.com/f5-cft/QA/azure-environment-2.0.json -o ${TMP_DIR}/azure-environment.json
...
if [[ $(echo <DISAGGREGATOR> | grep -E 'DNS') ]]; then
    DEPLOY_GTM='Yes'
...
DEPLOY_PARAMS='{"stackType":{"value":"'"${STACK_TYPE}"'"},"bastionSshKey":{"value":"'"${SSH_KEY}"'"},"deployBigiq":{"value":"'"${DEPLOY_BIGIQ}"'"},"deployGtm":{"value":"'"${DEPLOY_GTM}"'"},"dnsLabel":{"value":"<RESOURCE GROUP>"},"bigiqPassword":{"value":"B!giq2017"},"gtmPassword":{"value":"B!gdns2017"},"bigiqLicenseKey":{"value":"'"${BIG_IQ_LICENSE}"'"},"gtmLicenseKey":{"value":"'"${GTM_LICENSE}"'"},"licensePoolKeys":{"value":"'"${BIG_IQ_LICENSE_POOL}"'"}}'

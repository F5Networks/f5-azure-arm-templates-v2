...
if [[ <USE DEFAULT PARAMETERS> == 'Yes' ]]; then
    DEPLOY_PARAMS='{"secretId":{"value":"'"${SECRET_ID}"'"},"uniqueString":{"value":"<RESOURCE GROUP>"},"sshKey":{"value":"'"${SSH_KEY}"'"},"restrictedSrcAddressApp":{"value":"'"${SRC_IP}"'"},"restrictedSrcAddressMgmt":{"value":"'"${SRC_IP}"'"},'${BIGIQ_PARAMS}'}'
else
